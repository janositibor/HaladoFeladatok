...
class ChildDAOTest {
    ...
    @Test
    @DisplayName("Children Born After Year")
    void findChildrenBornAfterYearTest() {
        List<Child> expected = childDao.findChildrenBornAfterYear(2016);

        assertEquals(8, expected.size());
    }

    @Test
    @DisplayName("Child By Parent Name And Year Of Birth")
    void findChildByParentNameAndYearOfBirthTest() {
        Child expected = childDao.findChildByParentNameAndYearOfBirth("person1", 2015);

        assertEquals("child3", expected.getName());
    }

    @Test
    @DisplayName("Children With Max Sibling")
    void findChildrenwithMaxSiblingTest() {
        List<Child> expected = childDao.findChildrenWithMaxSibling();
