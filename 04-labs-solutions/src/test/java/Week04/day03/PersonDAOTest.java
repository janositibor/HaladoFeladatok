...
class PersonDAOTest {
    ...
    @Test
    @DisplayName("People With At Least Two Children")
    void PeopleWithAtLeastTwoChildrenTest() {
        List<Person> expected = personDao.findPeopleWithAtLeastTwoChildren();

        assertEquals(3, expected.size());
    }

    @Test
    @DisplayName("Person With Max Children")
    void PersonWithMaxChildrenTest() {
        Person expected = personDao.findPersonWithMaxChildren();

        assertEquals("person1", expected.getName());
    }

    @Test
    @DisplayName("Person By Child Name")
    void PersonByChildNameTest() {
        Person expected = personDao.findPersonByChildName("child6");

        assertEquals("person2", expected.getName());
    }

    @Test
    @DisplayName("Average Number Of Children")
    void AverageNumberOfChildrenTest() {
        double expected = personDao.findAverageNumberOfChildren();
